...
| `fullArtefactURI` | String | The ontology IRI carried by the consolidated OWL-full artefact (see `generateOWLFull`). Defaults to the canonical `base-ontology-uri`, so the consolidated form is published as the ontology itself. | `$base-ontology-uri`
...
| `generateOWLFull` | Boolean | Enables the optional consolidated OWL-full form (`make owl-full`): the OWL core and OWL restrictions artefacts are merged into a single self-contained ontology and the standalone outputs are dropped. See xref:user-guide/how-to-use.adoc#sec:owl-full[the OWL-full consolidated form] in the user guide. | `false`
|===
...
| `ontologyDescriptionFull` | String | Description of the consolidated OWL-full artefact. (dct:description) Optional; when omitted or blank it falls back to `ontologyDescriptionCore`. | _Some text_
...
| `ontologyLabelFull` | String | Label for the consolidated OWL-full artefact. (rdfs:label) Optional; when omitted or blank it falls back to `ontologyLabelCore`. | _Some text_
...
| `ontologyTitleFull` | String | Title for the consolidated OWL-full artefact. (dct:title) Optional; when omitted or blank it falls back to `ontologyTitleCore`. | `ePO Core full`
