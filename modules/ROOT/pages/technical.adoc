...
NOTE: If you don't have a virtual environment set up use "make create-virtual-env" to create a virtual environment and then activate it by using "source model2owl-venv/bin/activate"
...
NOTE: Do not change the values from the namespacePrefixes, umlDataTypesMapping, xsdAndRdfDataTypes variables as these will already work with having one config folder with all config files.
