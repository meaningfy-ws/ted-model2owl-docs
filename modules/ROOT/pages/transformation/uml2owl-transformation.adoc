...
Publications Office of the European Union began a project in 2015 to develop an eProcurement ontology, the motivation for which can be read in the xref:ePO_Arch_Design.adoc[eProcurement Ontology Architecture and Formalisation Specifications]. The ultimate objective of the project is to produce a commonly agreed ontology that will conceptualise, formally encode, and make available in an open, structured and machine-readable format, data about public procurement. The data will cover procurement end-to-end, i.e., from notification, through tendering to awarding, ordering, invoicing, and finally, payment.

The process and the methodology adopted involve crafting the conceptual model in https://www.omg.org/spec/UML/2.5.1[Unified Modelling Language (UML)] and then, by abiding to a set of conventions and recommendations, transforming that model into a formal ontology expressed in https://www.w3.org/OWL/[Web Ontology Language (OWL)] .

This document provides a working definition of the transformation rules for converting the UML conceptual model into the formal ontology and validation data shapes. These rules are organised in accordance with the xref:ePO_Arch_Design.adoc[eProcurement Ontology Architecture and Formalisation Specification].

=== Current Knowledge

Much has been written about https://www.e-informatyka.pl/index.php/einformatica/volumes/volume-2019/issue-1/article-3/[transformations from UML to OWL and vice versa]. The most significant literature on this topic was published between 2006 and 2019 comprising three book chapters, nine journal papers and multiple conference papers.
...
The remainder of this document comprises four sections covering major UML aspects. The section xref:transformation/transf-rules1.adoc#sec:tran-rules[Transformation of UML classes and attributes] treats classes and attributes. The next section deals with the  employed in the eProcurement model namely, xref:transformation/transf-rules2.adoc[associations, dependencies and generalisations]. The third section  describes the xref:transformation/transf-rules3.adoc[transformation rules] i.e., how the datatypes and enumerations should be transformed and finally, the forth and final section provides some xref:transformation/transf-rules4.adoc[transformation rules applicable to all UML elements], concerned with comments, labels and notes.
...
Each transformation rule is accompanied by the formal OWL representation in https://www.w3.org/TR/turtle/[Turtle]  and https://www.w3.org/2001/sw/RDFCore/TR/WD-rdf-syntax-grammar-20030117/[RDF/XML syntaxes] corresponding to the depicted UML fragment from the preceding figure. RDF/XML is a syntax that expresses RDF graphs as an XML document. Turtle is a textual syntax for RDF that is compact and resembles a natural text form with abbreviations for common usage patterns and datatypes.
...
<<#fig:uml-datatypes>> depicts a primitive datatype and an enumeration. No complex datatypes are depicted as they are treated in the same manner as classes.
