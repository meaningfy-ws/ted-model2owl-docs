...
|Disjoint generalization set | | |<<rule:disjoint-classes-rc>> |
...
A UML generalisation set [xref:references.adoc#ref:uml2.5[uml2.5]] groups generalisations. Subclasses in a generalization set are, by default, incomplete and can overlap. Of the generalization-set constraints, `{disjoint}` is transformed into OWL disjointness axioms (see <<rule:disjoint-classes-rc>>); the `{complete}` (covering) constraint is not transformed.
...
[[sec:disjoint-generalisation]]
=== Disjoint generalization sets

A UML generalization set may carry a `{disjoint}` constraint (`isDisjoint = true`),
asserting that no instance of the shared superclass belongs to more than one of the
listed subclasses. Model2owl transforms this constraint into OWL disjointness axioms.

[NOTE]
====
Only members of the specific generalization set are affected; if the
superclass has additional subclasses outside this set they are unaffected. This
supports _partial_ disjointness between named subsets of siblings.
====

[#rule:disjoint-classes-rc,source,XML,caption='',title='{example-caption} {counter:rule-cnt:2.1}. Disjoint classes from a `{disjoint}` generalization set -- in reasoning layer',reftext='{example-caption} {rule-cnt}']
====
For a generalization set marked `{disjoint}` (`isDisjoint = true`):

* two member subclasses → generate an `owl:disjointWith` axiom on each subclass;
* three or more member subclasses → generate a single `owl:AllDisjointClasses` axiom
  listing all members.
====
...
|===
|
.Disjoint pair (2 members) in Turtle syntax
[source,Turtle]
----
:SubClassA owl:disjointWith :SubClassB .
----

.Disjoint set (3+ members) in Turtle syntax
[source,Turtle]
----
[] a owl:AllDisjointClasses ;
   owl:members ( :SubClassA :SubClassB :SubClassC ) .
----
|
.Disjoint pair (2 members) in RDF/XML syntax
[source,XML]
----
<owl:Class rdf:about="http://base.onto.uri/SubClassA">
    <owl:disjointWith rdf:resource="http://base.onto.uri/SubClassB"/>
</owl:Class>
----

.Disjoint set (3+ members) in RDF/XML syntax
[source,XML]
----
<owl:AllDisjointClasses>
    <owl:members rdf:parseType="Collection">
        <owl:Class rdf:about="http://base.onto.uri/SubClassA"/>
        <owl:Class rdf:about="http://base.onto.uri/SubClassB"/>
        <owl:Class rdf:about="http://base.onto.uri/SubClassC"/>
    </owl:members>
</owl:AllDisjointClasses>
----
|===
