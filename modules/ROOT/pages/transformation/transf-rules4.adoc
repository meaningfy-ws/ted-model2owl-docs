...
[NOTE]
====
This annotation is optional: it is controlled by the `annotateDefinedConceptsWithOntology`
configuration parameter (default `true`). Set it to `false()` to suppress
`rdfs:isDefinedBy` in the OWL artefacts. See
xref:user-guide/configuration-file.adoc#sec:isdefinedby-annotation[Annotating concepts with the ontology URI].
====
...
[NOTE]
====
This annotation is optional: it is controlled by the `annotateShaclConceptsWithOntology`
configuration parameter (default `true`). Set it to `false()` to suppress
`rdfs:isDefinedBy` in the SHACL shapes. See
xref:user-guide/configuration-file.adoc#sec:isdefinedby-annotation[Annotating concepts with the ontology URI].
====
