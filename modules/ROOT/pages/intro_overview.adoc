...
#explain importance of eproc (naming consistancy, translations, relationships etc)#
...
#add process#
 ...
 How we already use this for eProcurement. Explain that we use this approach to produce the core vocabulary of eProcurement and the different modules that extend this core, like eCatalogue, eContract, eFulfilment etc. (there are many more)
...
In short, Model2Owl is used to generate a Semantic data specification from the ePO Conceptual model. 
